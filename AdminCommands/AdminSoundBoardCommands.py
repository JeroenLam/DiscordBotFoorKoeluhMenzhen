...
# Add a audio file to the Music
class AdminMusAddCommand(BaseCommand):
    async def execute(self, message):
        # Storing path to soundboard files
        soundboardDir = os.path.join(os.path.dirname(__file__), '../Music/')
        argv = message.content.split(' ')
        ...
        if len(message.attachments):                                    # If there is an attachment
            ...
            if url[-4:] == '.mp3':                                          # verify the extension
                ...
                await message.channel.send('Added ' + fileName + ' to the music list!')
                print('Added ' + fileName + 'To the music list')
            else:
                await message.channel.send('We only accept .mp3 files')
        else:
            await message.channel.send('No valid attachments!')
    ...
    def help(self):
        return '<file.mp3> <-nn>: Adds the <file>.mp3 to the Music folder. If you use `-nn` then the volume will not be normalised.'

# Remove audio files to the Music
class AdminMusRmCommand(BaseCommand):
    async def execute(self, message):
        # Storing path to soundboard files
        soundboardDir = os.path.join(os.path.dirname(__file__), '../Music/')
        ...
        for arg in argv:
            ...
            for root, dirs, files in os.walk(soundboardDir):
                for name in files:
                    if name == arg:
                        os.remove(soundboardDir + name)
                        await message.channel.send('Removed ' + name + ' from the music list!')           

    def help(self):
        return '<fileName_1> ... <fileName_N> : Remove <fileName_i>.mp3 from the music folder.'

# Rename audio files to the Music
class AdminMusMvCommand(BaseCommand):
    async def execute(self, message):
        # Storing path to soundboard files
        soundboardDir = os.path.join(os.path.dirname(__file__), '../Music/')
        argv = message.content.split(' ')
...
# Rename audio files to the Music
class AdminMusCpCommand(BaseCommand):
    async def execute(self, message):
        # Storing path to soundboard files
        soundboardDir = os.path.join(os.path.dirname(__file__), '../Music/')
        argv = message.content.split(' ')
